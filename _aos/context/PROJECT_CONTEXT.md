...
### WordPress / uPress — public market index (as of 2026-05-07)

- **AUTHORITATIVE RUNBOOK:** [`documentation/05-admin-and-operations/UPRESS_WP_REST_API_PUBLISH_RUNBOOK.md`](../../documentation/05-admin-and-operations/UPRESS_WP_REST_API_PUBLISH_RUNBOOK.md) — read first when touching publish/upload code, env vars, or operations. Covers: WP REST API primary path, FTPS defensive fallback, credentials structure (`UPRESS_WP_APP_USER`/`PASS`, app pwd label `sfa-pipeline`, user `agent`), mu-plugin `sfagent-allow-json.php` (manual install via uPress panel — port 21 is Bezeq-blocked), Mac↔server sync procedure, network reachability table, rotation procedure.
- **Primary upload path:** WP REST API (`/wp/v2/media`) over **HTTPS port 443**. Implementation: [`organic_market_agent/publisher/wp_upload.py`](../../organic_market_agent/publisher/wp_upload.py). FTPS retained only as opt-in fallback under `UPRESS_FALLBACK_FTPS=1`.
- **Why not FTPS:** Bezeq home-network egress blocks outbound port 21 entirely (verified Mac AND `waldhomeserver`, even after uPress IP whitelist). Pre-2026-05-07 publishes worked via Tailscale exit-node — removed because of repeated outages. WP REST is the permanent answer.
- **Architecture choice (AC-04 Option A):** Pipeline writes a `sfagent-manifest-of-urls.json` pointer file to WP media library; shortcode reads via WP option `sfagent_manifest_of_urls_url`, dereferences to artifact URLs. Decoupled from monthly WP date-based paths.
