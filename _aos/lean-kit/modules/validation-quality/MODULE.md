...
Universal hub validation via `validate_aos.sh`, L-GATE_BUILD exit criterion (PASS / 0 FAIL on applicable checks), and companion scripts (`validate_lod`, verdicts, gates). On **agents-os hub** with all `active_modules` enabled, the script runs **19 checks** (as of Lean Kit 3.1.7 / V320+).
