"""JmfFtNurseryseedingExtImporter — L14 FT_FINALE_NURSERYSEEDING (WP-C2 variant).

SFA-S003-P002-WP-C2 LOD400 §4.
Reads cached JSON from data/external_sources/extracted/jmf_ft_nurseryseeding_ext/.
FT table format (_table.json); English crop names resolved via JMF_CROP_MAP.
Populates: nursery_specific.

Note: distinct from WP-B2 JmfFtNurseryseedingImporter (name=jmf_ft_nurseryseeding_v1,
produces nursery_seeding_process). This C2 variant captures nursery-specific growing
notes rather than process instructions.
"""
...
class JmfFtNurseryseedingExtImporter(NIImporter):
    name = "jmf_ft_nurseryseeding_ext_v1"
    cache_dir = Path("data/external_sources/extracted/jmf_ft_nurseryseeding_ext")
    canonical_pdf_filename = "FT_FINALE_NURSERYSEEDING.PDF"
    _table_file = "_table.json"

    def _iter_cache_entries(self):
        """Yield (crop_jmf_en, notes_dict, provenance_dict) from _table.json."""
    ...
    def load_knowledge_notes(self, session) -> list[dict[str, Any]]:
        ...
        for crop_jmf_en, notes, provenance in self._iter_cache_entries():
            ...
            if len(body) > 2000:
                logger.warning(
                    "%s: %r body_text > 2000 chars — truncating", self.name, crop_jmf_en
                )
                body = body[:2000]
