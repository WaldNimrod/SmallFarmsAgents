"""JmfFtBiopesticideImporter — FT_TABLEAUAPPLICATIONBIOPESTICIPE (5pp).

SFA-S003-P002-WP-B2 LOD400 v1.1.3 §7.2.

Reads cached JSON from data/jmf/extracted/jmf_ft_biopesticide/.
Populates note_type='biopesticide_spray' only.
...
_SOURCE_NOTE_TYPE = "biopesticide_spray"


class JmfFtBiopesticideImporter(NIImporter):
    name = "jmf_ft_biopesticide_v1"
    cache_dir = Path("data/jmf/extracted/jmf_ft_biopesticide")
    canonical_pdf_filename = "FT_FINALE_TABLEAUAPPLICATIONBIOPESTICIPE.PDF"
    ...
    def load_knowledge_notes(self, session) -> list[dict[str, Any]]:
        """Return crop_knowledge_notes rows for biopesticide_spray."""
        rows = []
