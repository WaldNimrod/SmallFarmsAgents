"""JmfFtNurseryseedingImporter — FT_NURSERYSEEDING (13pp).

SFA-S003-P002-WP-B2 LOD400 v1.1.3 §7.2 (Q5 addition).

Reads cached JSON from data/jmf/extracted/jmf_ft_nurseryseeding/.
Populates note_type='nursery_seeding_process' only.
...
_SOURCE_NOTE_TYPE = "nursery_seeding_process"


class JmfFtNurseryseedingImporter(NIImporter):
    name = "jmf_ft_nurseryseeding_v1"
    cache_dir = Path("data/jmf/extracted/jmf_ft_nurseryseeding")
    ...
    def load_knowledge_notes(self, session) -> list[dict[str, Any]]:
        """Return crop_knowledge_notes rows for nursery_seeding_process."""
        rows = []
